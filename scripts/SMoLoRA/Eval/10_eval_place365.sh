...
if [ ! -n "$3" ] ;then
    RESULT_DIR="./results/SMoLoRA/Place365_single"
else
    RESULT_DIR=$3
fi
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.SMoLoRA.MET.model_vqa_cc_instruction \
        ...
        --ins-type 6 \
        --question-file /data/zqwang/moe_cl_data/CVIT_benchmark/Instructions_Single/Places365/val.json \
...
python llava/eval/SMoLoRA/Eval/eval_ImagetNet.py \
    --test-file /data/zqwang/moe_cl_data/CVIT_benchmark/Instructions_Single/Places365/val.json \
