...
if [ ! -n "$3" ] ;then
    RESULT_DIR="./results/SMoLoRA/Flickr30k_single"
else
    RESULT_DIR=$3
fi

for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.SMoLoRA.MET.model_caption \
        --model-path $MODELPATH \
        --model-base /data/zqwang/moe_cl_data/checkpoint/vicuna-7b-v1.5 \
        --ins-type 2 \
        --question-file /data/zqwang/moe_cl_data/CVIT_benchmark/Instructions_Single/Flickr30k/val.json \
...
python llava/eval/SMoLoRA/Eval/eval_flickr30k_cider.py \
